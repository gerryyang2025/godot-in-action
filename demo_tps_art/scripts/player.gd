...
const PLAYER_MODEL_ALBEDO := preload("res://assets/reference/player/textures/player_robot_albedo.png")
const PLAYER_MODEL_NORMAL := preload("res://assets/reference/player/textures/player_robot_normal.png")
const PLAYER_MODEL_ORM := preload("res://assets/reference/player/textures/player_robot_orm.png")
...
func _apply_movement(delta: float) -> void:
	...
	if _aiming:
		var aim_target := _get_crosshair_target_point()
		var flat_target := aim_target - global_position
		flat_target.y = 0.0
...
func _fire() -> void:
	var hit_position := _get_crosshair_target_point()

	var muzzle_position := _muzzle.global_position
	var shoot_direction := (hit_position - muzzle_position).normalized()
	var projectile_start := muzzle_position + shoot_direction * MUZZLE_EXIT_OFFSET
	var bullet = BULLET_SCENE.instantiate()
	get_tree().current_scene.add_child(bullet)
	bullet.configure(projectile_start, shoot_direction, self)
...
func _setup_player_model() -> void:
	...
	model_material.albedo_texture = PLAYER_MODEL_ALBEDO
	model_material.albedo_color = Color(0.96, 0.98, 1.0, 1.0)
	model_material.normal_enabled = true
	model_material.normal_texture = PLAYER_MODEL_NORMAL
	model_material.orm_texture = PLAYER_MODEL_ORM
	model_material.metallic = 0.3
	model_material.roughness = 0.98

	for mesh in _collect_model_meshes(_model_root):
		mesh.material_override = model_material


func _collect_model_meshes(root: Node) -> Array[MeshInstance3D]:
	var result: Array[MeshInstance3D] = []
	for child in root.get_children():
		if child is MeshInstance3D and child.mesh:
			result.append(child)
		result.append_array(_collect_model_meshes(child))
	return result


func _get_crosshair_target_point(max_distance: float = 200.0) -> Vector3:
	...
	if absf(ray_dir.y) > 0.001:
		var plane_distance := (global_position.y + 1.0 - ray_from.y) / ray_dir.y
		if plane_distance > 0.0:
			return ray_from + ray_dir * plane_distance
