...
func new_game() -> void:
	...
	_message_label.text = "Deployment shield active for 3 seconds. Sweep the training ground and eliminate all sentries."
	_update_hud()
...
func _show_ready_screen() -> void:
	...
	_message_label.text = "Press Space or Enter to deploy into the training ground. RMB or Q aims; LMB or F fires."
	_show_start_screen()
	_hide_end_screen()
	_update_hud()
...
func _finish_game(finish_reason: int) -> void:
	...
	if player_won:
			_message_label.text = "Training ground cleared. Press Space to redeploy."
		...
		_message_label.text = "Time limit exceeded. Press Space to retry."
			...
			_message_label.text = "You fell out of the arena. Press Space to retry."
	else:
...
func _show_end_screen() -> void:
	...
	match _finish_reason:
		...
		FinishReason.FELL:
			...
			report_summary = "The training run ended after the operator dropped out of the live-fire training ground."
