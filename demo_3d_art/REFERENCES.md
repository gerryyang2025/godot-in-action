...
- `godot-demo-projects-4.2-31d1c0c/3d/platformer/stage/stage.tscn`
  - 参考点：`WorldEnvironment`、程序天空、雾、3D 关卡轮廓和 DirectionalLight3D 的组合方式。
- `godot-demo-projects-4.2-31d1c0c/3d/voxel/default_env.tres`
  - 参考点：环境资源里 `glow_enabled`、`fog_enabled`、`fog_aerial_perspective` 等属性写法。
...
- `godot-docs-stable/classes/class_torusmesh.rst`
  - 参考点：信标 halo、无人机危险环和中心装置使用 `TorusMesh` 的半径/segments 写法。
- `godot-docs-stable/classes/class_environment.rst`
  - 参考点：程序天空、ambient、fog、glow、tonemap 的环境配置项。
