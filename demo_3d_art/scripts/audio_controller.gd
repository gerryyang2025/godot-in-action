...
var _scramble_fire_stream: AudioStreamWAV
var _scramble_denied_stream: AudioStreamWAV
var _hit_stream: AudioStreamWAV
var _reinforcement_stream: AudioStreamWAV
...
@onready var _ability_player: AudioStreamPlayer = $AbilityPlayer
...
func _exit_tree() -> void:
	...
	_deploy_stream = null
	_beacon_stream = null
	_scramble_fire_stream = null
	_scramble_denied_stream = null
	_hit_stream = null
	_reinforcement_stream = null
...
func play_deploy_confirm() -> void:
	...
	_play_one_shot(_ui_player, _deploy_stream, 1.0, -3.0)
...
func play_beacon_collect(progress: float) -> void:
	...
	_play_one_shot(_beacon_player, _beacon_stream, 1.0 + progress * 0.18, -4.0)


func play_scramble_fire() -> void:
	if not _audio_enabled:
		return

	_play_one_shot(_ability_player, _scramble_fire_stream, 1.0, -2.5)


func play_scramble_denied() -> void:
	if not _audio_enabled:
		return

	_play_one_shot(_ui_player, _scramble_denied_stream, 1.0, -7.0)
...
func play_reinforcement_alert() -> void:
	if not _audio_enabled:
		return

	_play_one_shot(_alert_player, _reinforcement_stream, 1.0, -2.0)
...
func play_countdown_warning(seconds_left: int) -> void:
	...
	var pitch := 1.0 + float(max(0, 10 - seconds_left)) * 0.04
...
func _build_streams() -> void:
	...
	_deploy_stream = _generate_sequence_stream([74, 81], 0.1, 0.18)
	_beacon_stream = _generate_chirp_stream(0.22, 620.0, 1320.0, 0.34, 0.24, 0.01)
	_scramble_fire_stream = _generate_chirp_stream(0.38, 210.0, 980.0, 0.36, 0.32, 0.04)
	_scramble_denied_stream = _generate_chirp_stream(0.12, 320.0, 210.0, 0.18, 0.12, 0.0)
	_hit_stream = _generate_hit_stream()
	_reinforcement_stream = _generate_sequence_stream([62, 69, 62, 74], 0.11, 0.2)
	_warning_stream = _generate_chirp_stream(0.1, 820.0, 900.0, 0.16, 0.08, 0.0)
...
func _generate_briefing_music_stream() -> AudioStreamWAV:
	...
	var roots: Array[int] = [45, 43, 41, 38]
	var arp_pattern: Array[int] = [0, 7, 12, 7, 3, 10, 12, 10]
	...
	for i in range(sample_count):
		...
		var bass := sin(TAU * t * bass_freq) * (0.14 + (0.5 + 0.5 * sin(TAU * t * 0.18)) * 0.04)
		var arp_env := pow(maxf(0.0, 1.0 - arp_step), 1.8)
		var arp := sin(TAU * t * arp_freq) * 0.1 * arp_env
		var glass := sin(TAU * t * arp_freq * 2.0) * 0.032 * arp_env
		var beat_phase := fmod(beat, 1.0)
		var pulse_env := pow(maxf(0.0, 1.0 - beat_phase * 3.0), 3.0)
		var pulse := _hash_noise(float(i) * 0.37) * 0.028 * pulse_env
		var hum := sin(TAU * t * 0.09) * 0.04
		samples[i] = clampf((bass + arp + glass + pulse + hum) * 0.82, -1.0, 1.0)
...
func _generate_gameplay_music_stream() -> AudioStreamWAV:
	...
	var lead_pattern: Array[int] = [12, 7, 10, 7, 14, 7, 10, 5]
	var samples := PackedFloat32Array()
	samples.resize(sample_count)
	...
	for i in range(sample_count):
		...
		var bass_env := pow(maxf(0.0, 1.0 - step_phase * 1.45), 1.6)
		var bass := sin(TAU * t * bass_freq) * 0.17 * bass_env
		var lead_freq := _midi_to_freq(root + lead_pattern[step_index])
		var lead_env := pow(maxf(0.0, 1.0 - step_phase * 1.75), 1.7)
		var lead := sin(TAU * t * lead_freq) * 0.085 * lead_env
		var local_beat_time := fmod(t, seconds_per_beat)
		var kick_window := seconds_per_beat * 0.44
		var kick_ratio := clampf(local_beat_time / kick_window, 0.0, 1.0)
		var kick_env := pow(maxf(0.0, 1.0 - kick_ratio), 3.0)
		var kick_freq := lerpf(122.0, 52.0, kick_ratio)
		var kick := sin(TAU * kick_freq * local_beat_time) * 0.3 * kick_env
		var hat_phase := fmod(beat * 4.0, 1.0)
		var hat_env := pow(maxf(0.0, 1.0 - hat_phase * 8.0), 2.5)
		var hat := _hash_noise(float(i) * 1.87 + 11.0) * 0.028 * hat_env
		var drive := sin(TAU * t * (bass_freq * 0.5)) * 0.03
		samples[i] = clampf((kick + bass + lead + hat + drive) * 0.92, -1.0, 1.0)
...
func _generate_ambience_stream() -> AudioStreamWAV:
	...
	for i in range(sample_count):
		var t := float(i) / MIX_RATE
		var rumble := sin(TAU * t * 55.0) * 0.08
		var drone := sin(TAU * t * 82.0) * 0.05 * (0.55 + 0.45 * sin(TAU * t * 0.14))
		var hiss := _hash_noise(float(i) * 0.41 + 5.0) * 0.016 * (0.55 + 0.45 * sin(TAU * t * 0.11))
		samples[i] = clampf(rumble + drone + hiss, -1.0, 1.0) * 0.75
...
func _generate_chirp_stream(duration: float, start_freq: float, end_freq: float, amplitude: float, harmonic: float, noise_amount: float) -> AudioStreamWAV:
	...
	for i in range(sample_count):
		...
		sample += _hash_noise(float(i) * 0.73 + 19.0) * noise_amount * env
...
func _generate_hit_stream() -> AudioStreamWAV:
	...
	for i in range(sample_count):
		var ratio := float(i) / float(max(1, sample_count - 1))
		var freq := lerpf(220.0, 48.0, ratio * ratio)
		phase = fmod(phase + freq / MIX_RATE, 1.0)
		var env := pow(maxf(0.0, 1.0 - ratio), 3.0)
		var tonal := sin(TAU * phase) * 0.28 * env
		var crackle := _hash_noise(float(i) * 2.13 + 3.0) * 0.18 * env
...
func _generate_sequence_stream(notes: Array[int], note_duration: float, amplitude: float) -> AudioStreamWAV:
	...
	for note_index in range(notes.size()):
		...
		for sample_index in range(start_sample, end_sample):
			...
			phase = fmod(phase + freq / MIX_RATE, 1.0)
			var sample := sin(TAU * phase) * amplitude * env
			sample += sin(TAU * phase * 2.0) * amplitude * 0.2 * env
