...
- 已完成：独立项目目录、`ART_DIRECTION.md` / `DESIGN.md` / `REFERENCES.md`、命名 `StandardMaterial3D` 资源、`WorldEnvironment` + 程序天空 + 雾 + glow、玩家/信标/无人机轮廓升级、收敛后的边缘光栏、控制台风格 HUD、开始说明页、成功/失败结算面板、运行时生成的占位 BGM / 环境音 / 关键事件音效骨架、屏幕闪光反馈、`Q` 干扰脉冲的对象级扩散特效、两条 headless 验证。
- 待补强：性能预算表、无人机地面路径投影、落地与信标收集等更多对象级事件特效、正式音频素材替换与混音细调、AA 对比记录、世界空间提示、完整人工可读性回归。
...
- 无人机：已具备机身、翼片、旋翼、前向传感器、危险环，以及可区分的三类轮廓。当前玩法已升级为 7 架开局无人机加 5 架分段增援机，其中巡逻型负责维持常规路线并投下圆环警戒线、猎手型会进入更强的持续追击并把锁定信息接力给附近猎手，同时投下箭头投影、封锁型负责扫荡关键通道并用双轨道投影压缩路线；三类单位还拥有不同的近距离占位音色。玩家现已具备“利用掩体打断视线”与“干扰脉冲”两种脱离追击手段，但尚未把整条巡逻路径投影到地面。
...
- 音频：已具备 `Music` / `Ambience` / `SFX` / `UI` bus 规划，以及开始部署、信标收集、干扰脉冲、受击、增援、倒计时和结算的占位音频；追逐态分层音乐和 3D 空间音效仍待补。
...
- [x] 已补上开始页、游玩态与结算态的占位音频骨架，并覆盖部署、收集、技能、受击、增援和倒计时关键音效。
- [ ] 场地灯色或 HUD 状态转场还可以更强。
...
- [x] 已搭好自定义 bus 和音频控制节点，后续可以在不改事件接线的前提下替换为正式音乐和音效素材。
