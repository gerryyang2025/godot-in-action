...
@onready var _gun_mesh: MeshInstance3D = $Visual/Gun/GunMesh
@onready var _collision_shape: CollisionShape3D = $CollisionShape3D
@onready var _hitboxes_root: Node3D = $Hitboxes
@onready var _muzzle: Marker3D = $Visual/Gun/Muzzle
@onready var _charge_orb: MeshInstance3D = $Visual/Gun/Muzzle/ChargeOrb
...
func _capture_unique_materials() -> void:
	...
	for mesh in [_body_mesh, _head_mesh, _gun_mesh]:
		...
		if material is StandardMaterial3D:
			...
			mesh.set_surface_override_material(0, instance_material)
